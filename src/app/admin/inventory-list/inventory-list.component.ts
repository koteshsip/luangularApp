...
export class InventoryListComponent implements OnInit {
  ...
  constructor(private dialogService:DialogService,public router: Router
    ,public mystorage:LocalStorageService,public inventoryService:InventoryService) { 
...
addInventory(){
  this.router.navigate(['/admin/add-inventory']);
}

getAllExams(event,filter){
this.message=this.mystorage.get("message");
          this.mystorage.remove("message");
		this.inventoryService.getAllInventory(event,filter).subscribe((data:any)=>{
				if(data.error) { 
					alert('Server Error');
				} else {
  ...
  showConfirm(id) {
            let disposable = this.dialogService.addDialog(MessageComponent, {
                ...
                .subscribe((isConfirmed)=>{
                    ...
                    if(isConfirmed) {
                        this.inventoryService.deleteInventory(id).subscribe((data:any)=>{
                            this.getAllExams(1,'');
